...
void write_text_file(const std::filesystem::path& path, const std::string& text) {
    ...
    // fsync file contents before rename so crash cannot leave truncated final path.
    {
    ...
    if (ec) {
        std::filesystem::remove(tmp);
        throw std::runtime_error("Failed to rename temp file onto: " + path.string() + " (" + ec.message() + ")");
    }
