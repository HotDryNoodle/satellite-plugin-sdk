...
bool type_matches(const nlohmann::json& value, const std::string& schema_type) {
    if (schema_type == "number") { return value.is_number(); }
...
SchemaValidation validate_value(const nlohmann::json& instance,
    ...
    if (schema.contains("enum")) {
        ...
        if (!matched) { return fail(path + " is not in enum"); }
    }
    ...
    if (schema.contains("type")) {
        ...
        if (type_node.is_string()) {
            if (!type_matches(instance, type_node.get<std::string>())) {
                return fail(path + " expected type " +
            ...
            if (!matched) { return fail(path + " type mismatch"); }
        }
    }
    ...
    if (instance.is_number()) {
        ...
        if (schema.contains("minimum") &&
            value < schema.at("minimum").get<double>()) {
            return fail(path + " below minimum");
        }
        if (schema.contains("maximum") &&
            value > schema.at("maximum").get<double>()) {
    ...
    if (schema.contains("properties") && instance.is_object()) {
        ...
        for (auto it = properties.begin(); it != properties.end(); ++it) {
            ...
            if (!instance.contains(key)) { continue; }
            auto child =
                validate_value(instance.at(key), it.value(), path + "." + key);
    ...
    if (schema.value("additionalProperties", true) == false &&
        instance.is_object()) {
        // additionalProperties 缺省为 true（JSON Schema 惯例）。
        for (auto it = instance.begin(); it != instance.end(); ++it) {
            if (!schema.contains("properties") ||
                !schema.at("properties").contains(it.key())) {
...
SchemaValidation validate_against_schema(const nlohmann::json& instance,
                                         const nlohmann::json& schema) {
    if (!schema.is_object()) { return fail("schema must be an object"); }
...
SchemaValidation resolve_schema_for_version(
    ...
    if (!std::filesystem::exists(registry_path)) {
        return fail("schema_registry.json not found: " +
                    registry_path.string());
    }
    const auto registry = read_json_file(registry_path);
    if (!registry.contains("versions") ||
        !registry.at("versions").is_object()) {
    ...
    if (!versions.contains(schema_version) ||
        !versions.at(schema_version).is_object()) {
        return fail("schema_registry has no entry for schema_version=" +
                    schema_version);
    }
    const auto& entry = versions.at(schema_version);
    if (!entry.contains(kind) || !entry.at(kind).is_string()) {
        return fail("schema_registry version " + schema_version +
                    " missing kind=" + kind);
    }
    const auto relative = entry.at(kind).get<std::string>();
    const auto resolved = default_schemas_dir() / relative;
    if (out_schema_path != nullptr) { *out_schema_path = resolved; }
...
SchemaValidation validate_by_schema_version(const nlohmann::json& instance,
    ...
    // Lightweight C++ validator: required/type/enum/const/additionalProperties
    // only. Full Draft 2020-12 ($ref/oneOf/if-then) coverage is in
